...
class EmpleadoDAO:
    ...
    def __init__(self):
        ...
        def Create(self, Empleado):
            nameSecuen = ""
            if(self.conn.validateSequence(Empleado.__class__.__name__)):
                self.conn.reconnect()
                nameSecuen = self.conn.createSequence(Empleado.__class__.__name__)
            else:
                nameSecuen = "seq_{0}".format(Empleado.__class__.__name__)
                self.conn.reconnect()
