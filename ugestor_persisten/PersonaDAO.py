...
class PersonaDAO:
	...
	def Update(self, Persona):
		...
		""".format(self.conn.getSCHEMA(), Persona.getCedula(), Persona.getNombre(), Persona.getApellido(), Persona.getFechaNacimiento(), Persona.getSexo(), Persona.getId(), Persona.gettipoDocumento())
		try:
			cn = self.conn.getConnection()
			cur = cn.cursor()
			cur.execute(self.sql, )
